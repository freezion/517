...
- (void)viewDidLoad
{
    ...
    if ([UIScreen mainScreen].bounds.size.height == 480.0) {
        for (int i = 0; i < [dataList count]; i ++) {
            ...
            UILabel *lblFindCount = [[UILabel alloc] initWithFrame:CGRectMake((95.0 + 10.0) * (i % 3) + 10.0, 104.0 + 120.0 * j, 85.0, 15.0)];
            lblFindCount.text = [NSString stringWithFormat:@"发现次数  %@", model.viewCount];
            lblFindCount.font = [UIFont fontWithName:@"HelveticaNeue" size:13.f];
        ...
        for (int i = 0; i < [dataList count]; i ++) {
            ...
            UILabel *lblFindCount = [[UILabel alloc] initWithFrame:CGRectMake((105.0 + 2.0) * (i % 3) + 16.0, 120.0 + 150.0 * j, 90.0, 21.0)];
            lblFindCount.text = [NSString stringWithFormat:@"发现次数  %@", model.viewCount];
            lblFindCount.font = [UIFont fontWithName:@"HelveticaNeue" size:15.f];
