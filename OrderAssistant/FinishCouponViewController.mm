...
#import "FinishCouponViewController.h"
...
@interface FinishCouponViewController ()

@end

@implementation FinishCouponViewController
...
- (void) showRecive {
    ...
    if (userName) {
        NSString *ccode = [usernamepasswordKVPairs objectForKey:KEY_CCODE];
        NSArray *dataList = [CouponEntity getCouponListByCCode:ccode];
        if ([dataList count] > 0) {
            UIStoryboard *storyborad = [UIStoryboard storyboardWithName:@"MainStoryboard" bundle:nil];
            ReciveViewController *controller = [storyborad instantiateViewControllerWithIdentifier:@"ReciveViewController"];
            UINavigationController *navPersonalViewController=[[UINavigationController alloc] initWithRootViewController:controller];
            [self presentModalViewController:navPersonalViewController animated:YES];
        } else {
            UIAlertView *alert = [[UIAlertView alloc] initWithTitle:@"提示" message:@"您没有获奖纪录哦" delegate:self cancelButtonTitle:@"好" otherButtonTitles:nil, nil];
            [alert show];
        }
...
- (void)loadScrollViewWithPage:(NSUInteger)page
{
    ...
    FinishDetailViewController *controller = [self.viewControllers objectAtIndex:page];
    if ((NSNull *)controller == [NSNull null])
    {
        controller = [storyboard instantiateViewControllerWithIdentifier:@"FinishDetailViewController"];
